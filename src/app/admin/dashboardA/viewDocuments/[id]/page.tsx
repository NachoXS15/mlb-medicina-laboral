            ...
            <section className="w-10/12 m-auto min-h-screen flex items-center justify-center flex-col gap-3 font-main" id="docs">
                ...
                <section className='w-full h-[700px] border border-slate-300'>
                    {docs?.length == 0 ? (
                        ...
                        const selectedTypeField = TypeField.find(field => field.title === doc.type)
                        ...
                        return (
                            <div key={i} className='flex items-center justify-between bg-slate-100 border-b border-b-slate-400 p-5'>
                                ...
                                <span className={`px-5 py-2 ${selectedTypeField?.style} rounded`}>{selectedTypeField?.title}</span>
                                <div className="flex items-center gap-5 px-15">
                                    <span className='hove:scale-105 transition' title="Descargar"><DownloadButton size={24} filePath={doc.path_name} /></span>
                                    <span className="hover:scale-105 transition" title="Eliminar"><DeleteButton filePath={doc.path_name} id={doc.id} /></span>
                                </div>
                            </div>
                        )
                    })}
